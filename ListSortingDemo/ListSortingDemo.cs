...
namespace ListSort
{
    public class ListSortingDemo
    {
        private static void Main(string[] args)
        {
            ...
            Console.WriteLine("{{ {0} }}", string.Join(", ", sourceList));

            Console.WriteLine("\nSolution #1 - Sort and de-duplicate using the List type built-in methods");
            timer.Start();
            var orderedList = sourceList.Distinct().Order();
            timer.Stop();
            Console.WriteLine("{{ {0} }}", string.Join(", ", orderedList));
            Console.WriteLine($"Time: {timer.Elapsed.TotalMilliseconds:F3}ms");
            ...
            timer.Stop();
            Console.WriteLine("{{ {0} }}", string.Join(", ", listAsSortedSet));
            Console.WriteLine("Total Time: {0:F3}ms", timer.Elapsed.TotalMilliseconds);
            ...
            Console.WriteLine("{{ {0} }}", string.Join(", ", listAsHash));
            Console.WriteLine("Time: {0:F3}ms", dedupeTime);
            ...
            Console.WriteLine("Time: {0:F3}ms", sortTime);
            Console.WriteLine("Total Time: {0:F3}ms", dedupeTime + sortTime);
        }
        ...
        public static List<int> CreateRandomListWithDuplicates(int distinctSize, int min, int max)
        {
            ...
            //Resize list to expected size
            if (random.Count() > distinctSize)
            {
                random.RemoveRange(distinctSize, random.Count() - distinctSize);
            }
